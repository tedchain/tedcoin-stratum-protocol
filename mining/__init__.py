...
def setup(on_startup):
    ...
    *before* you call setup() in the launcher script.'''
    ...
    bitcoin_rpc = BitcoinRPC(settings.TEDCOIN_TRUSTED_HOST,
                             settings.TEDCOIN_TRUSTED_PORT,
                             settings.TEDCOIN_TRUSTED_USER,
                             settings.TEDCOIN_TRUSTED_PASSWORD)

    import stratum.logger
    log = stratum.logger.get_logger('mining')

    log.info('Waiting for tedcoin RPC...')

    while True:
        try:
            result = (yield bitcoin_rpc.getblocktemplate())
            if isinstance(result, dict):
                log.info('Response from tedcoin RPC OK')
                break
        except:
            time.sleep(1)
